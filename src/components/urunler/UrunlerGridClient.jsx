...
const STRIP_CLIP = "polygon(8% 0, 100% 0, 92% 100%, 0 100%)";
...
export default function UrunlerGridClient({ products }) {
  const sortedProducts = useMemo(() => {
    ...
    return [...items].sort((a, b) => {
      ...
      const tA =
        (typeof a?.title === "string" ? a.title : a?.title?.tr || a?.title?.en || "") || "";
      const tB =
        (typeof b?.title === "string" ? b.title : b?.title?.tr || b?.title?.en || "") || "";
      return tA.localeCompare(tB, "tr", { sensitivity: "base", numeric: true });
    });
  }, [products]);
    ...
    return (
      <section className="max-w-[1200px] mx-auto px-6 lg:px-10 py-16 bg-[#0A0F1A]">
        <div className="rounded-xl border border-white/10 bg-white/90 p-10 text-center">
          <p className={`${rajdhani.className} text-[#0B1F3A] text-lg tracking-wide`}>
      ...
      <div className="relative max-w-[1200px] mx-auto px-6 lg:px-10 py-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12">
          {sortedProducts.map((p) => {
            ...
            const img =
              primary &&
              urlFor(primary)?.width(800)?.height(640)?.fit("max")?.auto("format")?.url();

            return (
              <Link
                key={p?._id || slug}
                href={slug ? `/urunler/${slug}` : "#"}
              ...
              >
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      background:
                        "linear-gradient(90deg,#2BA84A 0%,rgba(0,0,0,.1) 50%,#102C8C 100%)",
                    }}
                  />
                  ...
                  <div className="relative w-full bg-white">
                    ...
                    <div className="absolute top-3 left-3">
                      <span
                        className={`${rajdhani.className} inline-flex items-center text-[10px] font-semibold uppercase tracking-[0.08em] text-[#0B1F3A]`}
                        style={{ background: "#EAFBF0", clipPath: CHIP_CLIP }}
                      >
                        <span className="px-2 py-[6px]">Optimus&nbsp;Prime</span>
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="px-4 md:px-5 pt-3 pb-4">
                    <h3
                      className={`${rajdhani.className} text-[#0B1F3A] text-lg md:text-[20px] font-semibold uppercase tracking-[0.04em] line-clamp-2`}
                    >
                      {title}
                    </h3>
                    <div className="mt-1 text-[11px] text-gray-500 flex items-center justify-between">
                      <span>Endüstriyel Motor</span>
                      <span className="opacity-0 transition-opacity group-hover:opacity-100 text-[#102C8C]">
                        İncele →
                      </span>
                    </div>
                  </div>
  ...
  style={{
    background:
      "linear-gradient(90deg,rgba(43,168,74,.9),rgba(43,168,74,.25),rgba(16,44,140,.9))",
  }}
/>
                </article>
              </Link>
